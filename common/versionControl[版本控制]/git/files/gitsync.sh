...
CUR_BRANCH_SHOW_NAME=`git branch -vv |awk '$1 == "*" {print $2}'`
[ $DEBUG_LOG ] && echo "[DEBUG] CUR_BRANCH_SHOW_NAME=$CUR_BRANCH_SHOW_NAME"

CUR_BRANCH=`git branch -vv |awk '$1 == "*" {print $4}'`
[ $DEBUG_LOG ] && echo "[DEBUG] step 0 | CUR_BRANCH=$CUR_BRANCH"
# 掐头去尾，去掉最外面的[]
CUR_BRANCH=${CUR_BRANCH:1:`expr ${#CUR_BRANCH}-2`}
[ $DEBUG_LOG ] && echo "[DEBUG] step 1 | CUR_BRANCH=$CUR_BRANCH"
# 第一个/左边的是remote名称，一般是origin
REMOTE_NAME=${CUR_BRANCH%%/*}
# 第一个/右边的内容，可能是"分支名: ahead 1, behind 4"
CUR_BRANCH=${CUR_BRANCH#*/}
[ $DEBUG_LOG ] && echo "[DEBUG] step 2 | CUR_BRANCH=$CUR_BRANCH"
# 第一个:左边的内容，最终取到分支名
CUR_BRANCH=${CUR_BRANCH%%:*}
[ $DEBUG_LOG ] && echo "[DEBUG] step 3 | CUR_BRANCH=$CUR_BRANCH"
...
then
    INPUT_BRANCH=`git branch -vv |awk -v AWK_BRANCH="$1" '$1 == AWK_BRANCH || ( $1 == "*" && $2 == AWK_BRANCH ) {print $3}'`
    [ $DEBUG_LOG ] && echo "[DEBUG] search in git branch -vv: INPUT_BRANCH='$INPUT_BRANCH'"
    if [ "$INPUT_BRANCH" != "" ]
    then
        # 掐头去尾，去掉最外面的[]
    ...
    [ $DEBUG_LOG ] && echo "[DEBUG] INPUT_BRANCH=$INPUT_BRANCH"
